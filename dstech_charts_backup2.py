...
def create_chemical_consumption_chart(start_date=None, end_date=None):
    ...
    # Converter para numérico e tratar valores inválidos
    df['quimico_1'] = pd.to_numeric(df['quimico_1'], errors='coerce').fillna(0)
    df['quimico_2'] = pd.to_numeric(df['quimico_2'], errors='coerce').fillna(0)
    df['quimico_3'] = pd.to_numeric(df['quimico_3'], errors='coerce').fillna(0)
    df['quimico_4'] = pd.to_numeric(df['quimico_4'], errors='coerce').fillna(0)
    df['production_weight'] = pd.to_numeric(df['production_weight'], errors='coerce').fillna(1)
    
    # Cálculos de consumo por quilo conforme README
    # Evitar divisão por zero
    df['production_weight'] = df['production_weight'].replace(0, 1)
    df['quimico_1_per_kg'] = (df['quimico_1'] / df['production_weight']).round(3)
    df['quimico_2_per_kg'] = (df['quimico_2'] / df['production_weight']).round(3)
    df['quimico_3_per_kg'] = (df['quimico_3'] / df['production_weight']).round(3)
    df['quimico_4_per_kg'] = (df['quimico_4'] / df['production_weight']).round(3)
    
    fig = go.Figure()
    
    fig.add_trace(go.Scatter(
        x=df['timestamp'], y=df['quimico_1_per_kg'],
        mode='lines+markers', name='Detergente',
        line=dict(color='#e74c3c', width=2)
    ))
    
    fig.add_trace(go.Scatter(
        x=df['timestamp'], y=df['quimico_2_per_kg'],
        mode='lines+markers', name='Amaciante',
        line=dict(color='#9b59b6', width=2)
    ))
    
    fig.add_trace(go.Scatter(
        x=df['timestamp'], y=df['quimico_3_per_kg'],
        mode='lines+markers', name='Alvejante',
        line=dict(color='#f39c12', width=2)
    ))
    
    fig.add_trace(go.Scatter(
        x=df['timestamp'], y=df['quimico_4_per_kg'],
